...
`anomaly_score` is **anomaly-oriented**: higher values are more anomalous. It is the negated
IsolationForest decision function, `-decision_function(x)`, so the model's own decision boundary
sits at zero — a score above `0` is an outlier, `0` or below an inlier. `is_anomaly` comes from the
model's prediction (`-1` for an outlier), not from a threshold applied by the caller.

The score is a model-derived **ranking score, not a probability**. It is unbounded and is
deliberately not squashed into `[0, 1]`, because a bounded number invites being read as a
likelihood it is not. The API exposes this orientation rather than scikit-learn's native one, whose
sign convention is the opposite way round.
...
Training lives in `inference-service/ml/`: `generate_training_data.py` produces deterministic
synthetic samples of normal operation in canonical units, and `train.py` fits IsolationForest and
writes the artifact plus its metadata. The artifact is git-ignored and rebuilt from the script.

Some features are deliberately coupled in the generated data — pack voltage tracks state of charge,
motor speed tracks road speed, current tracks load — so the model learns a joint structure rather
than six independent ranges, and can flag a sample whose values are each individually plausible but
jointly impossible.

**What this model is and is not.** IsolationForest was chosen because anomaly labels for fleet
telemetry are scarce and this project demonstrates an end-to-end deployment lifecycle rather than
maximizing predictive accuracy. The synthetic dataset, the feature ranges and the contamination
threshold are demonstration assumptions, **not validated against any production EV platform**.
IsolationForest also cannot extrapolate: a sample beyond the training range falls into the same
leaf as the boundary samples, so a single wildly out-of-range feature is not guaranteed to be
flagged. The system records what produced each verdict through `model_name` and `model_version`
rather than implying accuracy it has not earned.
